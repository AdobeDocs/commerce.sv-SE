...
- **StoreFront-tjänster** - AI-baserade funktioner för produktupptäckt, rekommendationer och betalningar
- **Integreringstjänster** - Anslutningar till Adobe Experience Platform, AEM Assets och andra Adobe-lösningar
...
Förutom dessa funktioner finns det verktyg som gör att du kan övervaka dataflödet från din Commerce-instans till SaaS-plattformen. Dessa dataverktyg kan automatiskt synkronisera data och hjälpa dig att optimera prestandan.
...
<tr style="border: 0;">
   ...
   <td valign="top">
      ...
      <img alt="Apphantering" src="../assets/icons/DataSetting.svg" width="40">
      </a>
      <div>
         <a href="../app-management/overview.md">
      ...
      <p>
         <em>Associera, konfigurera och hantera App Builder-program med din Commerce-instans via administratörsgränssnittet.</em>
      </p>
   </td>
</tr>
</table>
...
## Hur Commerce Services samverkar

Adobe Commerce Services bygger på en enhetlig plattform. När du använder flera tjänster kan du dra nytta av:

- **Enhetlig dataregling** - Produkt-, prissättnings- och lagerdata synkroniseras automatiskt för alla tjänster, vilket eliminerar behovet av dubblerad datainmatning eller anpassade integreringar.
- **Realtidspersonalisering** - Kundbeteenden från sökning, surfning och inköp ger smarta rekommendationer och rankning i hela butiken.
- **Konsekvent integrering mellan butiker** - Oavsett om du använder Commerce storefront från Edge Delivery Services, PWA Studio eller headless-implementeringar fungerar samma plugin-program och API:er sömlöst på alla plattformar.
- **Stöd för flera butiker och flera språk** - Alla tjänster respekterar automatiskt dina butiksvyer, kundsegment och katalogkonfigurationer för B2B utan ytterligare konfiguration.
- **Detaljrik produktbild** - När du integrerar med AEM Assets visas optimerade produktbilder på ett enhetligt sätt i sökresultat, produktsidor och rekommendationer.
- **Anslutna kunddata** - Använd Dataanslutning för att dela shoppingbeteenden med Adobe Experience Platform, Real-Time CDP och Journey Optimizer, vilket möjliggör kanalövergripande personalisering och kampanjoptimering.

När en kund t.ex. söker efter en produkt med Live Search, lägger till den i kundvagnen efter att ha visat en produktrekommendation och slutför köpet med Payment Services, flödar all den här aktiviteten smidigt genom den enhetliga dataredningen. Dessa beteendedata ger bättre sökresultat och mer relevanta rekommendationer för framtida kunder.

>[!NOTE]
>
>Du måste konfigurera insamling av händelsedata för att aktivera beteendedelning av data mellan tjänster. Se [Konfigurera Commerce-tjänster](saas.md#saas-configuration) för installationsanvisningar.

Varje tjänst kan användas oberoende av varandra, men genom att kombinera den skapar du en mer intelligent och personaliserad shoppingupplevelse.
