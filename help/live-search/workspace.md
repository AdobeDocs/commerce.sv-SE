...
>[!TIP]
>
>Att välja vilka attribut som ska vara sökbara påverkar sökkvaliteten avsevärt. Mer information om hur du väljer sökbara attribut och undviker vanliga konfigurationsproblem finns i [Utnyttja produktmetadata](best-practice.md#leverage-product-metadata) i guiden om bästa praxis.

### Attributbeteende i komplexa produkter

För komplexa produkttyper (konfigurerbara, paketerade och grupperade produkter) indexerar [!DNL Live Search] attributvärden från både överordnade och underordnade produkter, vilket gör att en överordnad produkt kan associeras med flera värden för samma attribut. Detta möjliggör variantbaserad filtrering, t.ex. visas en konfigurerbar skjorta vid filtrering med &quot;blått&quot; om en variant är blå, även om den överordnade produkten inte har någon färginställning.

Detta fungerar bra för attribut som färg och storlek, men kan orsaka oväntade resultat för attribut som `new_arrival`, `product_ranking`, `promotion_label` eller anpassade prisattribut. Om en konfigurerbar produkt (SKU-001) till exempel har `new_arrival = true`, men dess underordnade variant (SKU-001-01) har `new_arrival = false`, indexeras den överordnade produkten SKU-001 med båda värdena (`true` och `false`) så att den kan visas i sökresultat för båda villkoren.
