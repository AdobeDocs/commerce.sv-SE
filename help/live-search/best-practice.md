...
- En väldesignad sökupplevelse skapar förtroende hos kunderna och ser till att de kan hitta det de behöver.
...
Att välja vilka attribut som ska vara sökbara påverkar sökkvaliteten avsevärt. Om du gör för många attribut sökbara kan det minska relevansen och orsaka oväntade träffar, även om antalet returnerade resultat ökar. I det här avsnittet beskrivs hur du avsiktligt väljer sökbara attribut för att balansera täckning och relevans.

**Rekommenderade sökbara attribut:**

- **Produktnamn** - Hög återgivning beskriver produkten direkt.
- **Primär beskrivning** - Exakt produktinformation som kunderna förväntar sig ska matcha.
- **Varumärke** - Shoppare söker ofta efter varumärke.
- **Modellnummer/format** - Specifika identifierare med klar avsikt.
- **Viktiga funktioner** - Viktiga särskiljande egenskaper (till exempel &quot;vattentät&quot;, &quot;trådlös&quot;).
- **Material/struktur** - För mode- och möbelkategorier.

**Undvik att göra dessa attribut sökbara:**

- **Långa beskrivningar eller specifikationer** - För mycket text skapar brus och oväntade matchningar.
- **Kategorisökvägar** - Kan orsaka irrelevanta resultat på grund av breda taxonomitermer.
- **Interna SKU-koder med blandade tecken** - Skapar falskt positiva för partiella matchningar.
- **Administrativa fält** - Interna anteckningar eller lagerställekoder är inte relevanta för kunder.
- **HTML-innehåll eller formateringstaggar** - Tekniskt innehåll ökar inte relevansen.

#### Vanliga problem orsakade av felaktiga sökbara attribut

Att göra fel attribut sökbara kan frustrera kunderna och skapa supporteskaleringar.

| Exempel | Scenario | Rekommendation |
|---------|----------|----------------|
| **Semma och komplettera automatiskt biverkningar** | En handlare gör långa produktbeskrivningar sökbara. En kund söker efter&quot;burk&quot; (letar efter behållare). På grund av ordstam och partiell matchning visas produkter med &quot;burk&quot; som en del av större ord i beskrivningarna, till exempel &quot;American&quot;, &quot;canopy&quot; eller &quot;canvas&quot;. Produkter som inte har med kundens avsikter att göra, och de tappar förtroendet för sökningen. | Minska antalet sökbara fält till intent-attribut som produktnamn och primär beskrivning. Validera de viktigaste söktermerna för att identifiera problematiska matchningar. Använd regler för sökmarknadsföring eller omdirigeringar för att hantera specifika kända kantfall. |
| **Rankning efter popularitet förstärker brusmatchning** | En handlare anger rangordningen till&quot;Mest köpta&quot; och inkluderar kategorisökvägar och långa beskrivningar som sökbara attribut. En kund söker efter en&quot;väska för bärbara datorer&quot;. Den breda matchningen returnerar väskor till bärbara datorer, tillbehör till bärbara datorer, väskor för andra ändamål samt bärbara datorer. Eftersom bärbara datorer köps oftare än väskor på bärbara datorer rankas de högst upp. Köparen uppfattar rankningen som felaktig, även om systemet fungerar som det är konfigurerat. | Ta bort brus sökbara attribut som kategorisökvägar. När matchningen är exaktare kan ni tillämpa popularitetsbaserade rankningsstrategier. Övervaka sökanalyser för att identifiera frågor där mönstret finns. |
| **Kategorisökvägen skapar falska positiva** | En handlare gör hela kategorisökvägen sökbar (t.ex. &quot;Hem > Kitchen > Appliances > Small Appliances&quot;). En köpare söker efter&quot;hemmakontor&quot;. Produkter i kategorin&quot;Hem&quot; matchar, även om de är köksartiklar, eftersom&quot;Hem&quot; finns i deras kategorisökväg. Köksapparater, heminredning och andra orelaterade produkter visas i resultat, vilket späder ut relevansen. | Gör inte kategorisökvägar sökbara. Använd ansikten i stället för att låta kunderna filtrera efter kategori. Om kategorifiltrering är viktigt för din sökstrategi ska du implementera det via försäljningsregler i stället för sökbara attribut. |

#### Viktattribut för sökbar text
...
- **Högsta vikt (9-10):** Produktnamn, varumärke
- **Medium weight-papper (6-8):** Modellnummer, primär beskrivning, nyckelfunktioner
- **Lägre vikt (3-5):** Sekundära beskrivningar, material, specifikationer
...
Om sökresultaten känns slumpmässiga eller irrelevanta kan du använda den här checklistan innan du eskalerar som en produktdefekt:

1. **Granska konfigurationen för sökbart attribut:**

   - Visa alla attribut som är inställda som sökbara.
   - Identifiera eventuella breda eller brusande attribut (långa beskrivningar, kategorisökvägar, administrativa fält).
   - Ta bort sökbar status från attribut som inte representerar kundavsikten.

1. **Verifiera frågematchningar mot huvudattribut:**

   - Testa dina vanligaste sökfrågor.
   - Verifiera att resultatet i första hand matchar produktnamn och märke i stället för tangentiell text.
   - Kontrollera om oväntade resultat bara delar svaga träffar i långt innehåll.

1. **Testa med och utan brusfält:**

   - Ta tillfälligt bort sökbar status från kategorisökvägar och långa beskrivningsfält.
   - Kör problematiska frågor igen för att se om relevansen förbättras.
   - Om resultatet förbättras kan du justera konfigurationen permanent.

1. **Använd försäljningsregler för undantag:**

   - Skapa riktade sökregler för specifika kända frågor som behöver specialhantering.
   - Försök inte att lösa kantfall genom att göra fler attribut sökbara.
   - Använd omdirigeringar för varumärkessökningar eller vanliga felstavningar.

1. **Övervaka och iterera:**

   - Använd [prestandaarbetsytan](performance.md) för att spåra nollresultatfrekvensen och klickfrekvensen.
   - Granska de vanligaste sökfrågorna varje vecka för att identifiera nya mönster.
   - Justera sökbara attribut och vikter baserat på data, inte antaganden.
